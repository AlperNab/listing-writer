...
**Domain:** `E-commerce / Product Content` · **Suite:** `E-commerce Growth Suite` · **Accent:** `#a3e635`
...
**Listing Writer** is a standalone, browser-based AI workflow app for **E-commerce / Product Content**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `product_details`<br>Product details | text | Yes | — | Affects input: Product details. |
...
| `marketplace`<br>marketplace | text | No | — | Affects customization: marketplace. |
| `category`<br>category | text | No | — | Affects customization: category. |
| `brand_voice`<br>brand voice | text | No | — | Affects customization: brand voice. |
| `target_customer`<br>target customer | text | No | — | Affects customization: target customer. |
| `keyword_list`<br>keyword list | text | No | — | Affects customization: keyword list. |
| `forbidden_claims`<br>forbidden claims | textarea | No | — | Affects customization: forbidden claims. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9132
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9132`. |
...
```bash
curl http://127.0.0.1:9132/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9132/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9132 uvicorn app.main:app --host 0.0.0.0 --port 9132
```
...
- `plugins/listing-writer.json` for plugin metadata.
